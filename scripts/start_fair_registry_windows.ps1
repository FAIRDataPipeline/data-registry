...
if ($REG_BACKGROUND) {
	Start-Process pwsh -ArgumentList 'Start-Process "${FAIR_HOME}\venv\Scripts\python -NoNewWindow -Args ${FAIR_HOME}\manage.py runserver ${FULL_ADDRESS}" -RedirectStandardError "${FAIR_HOME}\output_error.log" -RedirectStandardOutput "${FAIR_HOME}\output.log"'
} else {
