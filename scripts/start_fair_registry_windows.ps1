...
if ($REG_BACKGROUND) {
	cmd /c "START /b $FAIR_HOME\venv\Scripts\python ${FAIR_HOME}\manage.py runserver ${FULL_ADDRESS} 1> ${FAIR_HOME}\output.log 2>&1"
} else {
...
Start-Process "${FAIR_HOME}\venv\Scripts\python" -NoNewWindow -Args "${FAIR_HOME}\manage.py get_token" -RedirectStandardOutput "${FAIR_HOME}\token" -RedirectStandardError "${FAIR_HOME}\token-error"
