...
def find_object_by_file_hash(file_hash, opts):
    url = (
        API_ROOT
        + STORAGE_LOCATION_ENDPOINT
        + "?format=json&hash=%s" % html.escape(file_hash)
    )

    result = read_api(
        url, "Could not find storage location with file hash matching %s" % opts.file
    )
    ...
    url = (
        API_ROOT
        + OBJECT_ENDPOINT
        + "?format=json&storage_location=%d" % storage_location_id
    )
    object_data = read_api(
        url, "Could not find object with storage_location %d" % storage_location_id
    )
    ...
    namespace = read_api(
        ...
        "Failed to read namespace %s" % data_product["namespace"],
    )

    data_product_name = "%s::%s@%s" % (
        namespace["name"],
        data_product["name"],
...
def find_object_by_data_product(opts):
    url = (
        API_ROOT
        + DATA_PRODUCT_ENDPOINT
        + "?format=json&name=%s" % html.escape(opts.data_product)
    )
    if opts.namespace:
...
def find_namespace_id(opts):
    url = (
        API_ROOT
        + NAMESPACE_ENDPOINT
        + "?format=json&name=%s" % html.escape(opts.namespace)
    )
...
def check_file(opts):
    ...
    if opts.data_product:
        ...
        data_product_name, object_data = find_object_by_file_hash(
            file_hash.hexdigest(), opts
        )
...
def get_status(opts):
    ...
    storage_location_data = read_api(
        storage_location_url,
        "failed to read storage_location %s" % storage_location_url,
    )
...
def main(args=None):
    ...
    check_parser = subparsers.add_parser(
        "check", help="check file contains against registry"
    )
    check_parser.add_argument(
        "-d", "--data_product", type=str, help="Registry data product to look up"
    )
    check_parser.add_argument(
        "-n", "--namespace", type=str, help="Namespace of data product"
    )
    check_parser.add_argument("file", type=str, help="File to process")

    status_parser = subparsers.add_parser(
        "status", help="print data registry entry for data product"
    )
    status_parser.add_argument(
        "-n", "--namespace", type=str, help="Namespace of data product"
    )
    status_parser.add_argument(
        "data_product", type=str, help="Registry data product to look up"
    )
